...
describe('Ships the game', function () {
  it('should navigate to correct sites after clicking end the game button', function () {
    ...
    browser.wait(protractor.ExpectedConditions.urlIs("http://localhost:4200/game/DUMMY_PLAYER_1"), 3000).then(function () {
      browser.findElement(by.id('endGameButton')).click();
    });

    browser.sleep(8000).then(function () {
      Promise.all([
        expect(browser.getCurrentUrl()).toEqual('http://localhost:4200/landing/loose'),
        expect(browser2.getCurrentUrl()).toEqual('http://localhost:4200/landing/win')])
        .then(function () {
